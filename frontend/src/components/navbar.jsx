  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
      ...
      <ul className='hidden md:flex items-start gap-5 font-medium'>
        ...
        {/* ADMIN button opens in a new tab */}
        <li className='py-1'>
          <a
            href={ADMIN_URL}
            target='_blank'
            rel='noopener noreferrer'
            className='px-3 py-1 rounded border border-primary hover:bg-primary hover:text-white transition'
          >
            ADMIN
          </a>
        </li>
        ...
        <div className={`${showMenu?'fixed w-full' : 'h-0 w-0'} md:hidden bg-white overflow-hidden top-0 bottom-0 right-0 z-20 transition-all`}>
          ...
          <div>
            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium transition-all'>
              ...
              {/* Mobile ADMIN link (new tab) */}
              <a
                href={ADMIN_URL}
                target='_blank'
                rel='noopener noreferrer'
                className='px-4 py-2 rounded inline-block border border-primary'
