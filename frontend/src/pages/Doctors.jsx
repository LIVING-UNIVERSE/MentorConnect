  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        <button  className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter? 'bg-primary text-white':'text-black'}`} onClick={()=>setShowFilter(prev=>!prev)}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter?'flex' :'hidden sm:flex'}`}>
          <p
            ...
            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${
              speciality === 'General physician' ? 'text-black bg-indigo-100' : ''
            }`}
          >
            General physician
          </p>
          ...
          <p
            ...
            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${
              speciality === 'Gastroenterologist' ? 'text-black bg-indigo-100' : ''
            }`}
          >
            Gastroenterologist
          </p>
        ...
        <div className="w-full grid grid-cols-auto gap-4 gap-y-6">
          {filterDoc.map((item, index) => (
            <div
              ...
              key={index}
            >
              <img className="bg-blue-50 hover:bg-primary" src={item.image} alt="Doctor" />
              <div className="p-4">
